...
#define GRANT_X 1.5     // right in front of the robot's initial position
...
int main(int argc, char *argv[]) {
    ...
    move_base_feedback_msg.status.status = SUCCEEDED;
    ...
    double x = -5.0, y = 5.5, angle = 45.0;
    while (ros::ok() && x < GRANT_X) {
        x += 2.0;
        // wait for the previous goal to finish
        while (move_base_feedback_msg.status.status == PENDING || move_base_feedback_msg.status.status == ACTIVE || move_base_feedback_msg.status.status == PREEMPTING) {;}
        while (move_base_feedback_msg.status.status != SUCCEEDED) {;}
